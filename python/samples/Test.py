...
from Tale_class import RoboTale
...
Tale=RoboTale(device="0",model="Azure")
            ...
            async def collect_audio(audioContentPart: RTAudioContent):
                ...
                async for chunk in audioContentPart.audio_chunks() :
                    ...
                    while len(audio_data) >= sample_rate * channels * 2:
                        await asyncio.sleep(0.3)  # Allow the callback to process the audio data 
                ...
                while len(audio_data) > 0:
                    ...
                    if (time.time()-start_audio)>l*0.22:
                        if(text_chunk_index<len(sentence_dict)+1):
                            ...
                            text_chunk_index+=1 """
                    await asyncio.sleep(0.3)  # Allow the callback to process the audio data
                #while stream.active:  # Polling the active state
                    
                stream.stop()
                stream.close()
                stop_audio=time.time()
                print("speaking done")
                print("Count of audio ids",audio_id, "Speaking time: "  ,stop_audio-first_start_audio)
            ...
            async def collect_transcript(audioContentPart: RTAudioContent):
                ...
                async for chunk in audioContentPart.transcript_chunks():
                    ...
                    if chunk in [".", "?", "!",":"]:
                        ...
                        sentence_dict[new_key]={"sentence":sentence,"chunk_id":chunk_id,"sentence_emotion":predicted_label,"length":l}
                        sentence=""
                        k+=1
...
async def run(client: RTClient, instructions_file_path: str, out_dir: str):
    with open(instructions_file_path, encoding="utf-8") as instructions_file:
        ...
        while True:
            user_message=""
            while user_message=="":
                user_message=aS2T.recognize_from_microphone(lang,Tale)
            #user_message = input("Enter your message (type 'stop' to end): ")
            if "Stop." in user_message or "Goodbye." in user_message or "Bye." in user_message or "stop." in user_message or "goodbye." in user_message or "bye." in user_message:
                break
